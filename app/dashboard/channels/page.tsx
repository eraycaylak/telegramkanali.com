...
export default function MyChannelsPage() {
    ...
    async function fetchChannels() {
        try {
            ...
            // Always show user's own channels in dashboard
    ...
    if (loading) return <div className="space-y-6 animate-pulse">
        {[1, 2].map(i => <div key={i} className="h-40 bg-gray-100 rounded-3xl"></div>)}
    </div>;
                            ...
                            <div className="flex items-start gap-4">
                                <div className="relative">
                                    ...
                                    <div className="absolute -bottom-1 -right-1">
                                        {channel.bot_enabled ? (
                                            <div className="bg-blue-600 text-white p-1 rounded-lg shadow-lg border-2 border-white" title="Bot Aktif">
                                                <RefreshCw size={10} />
                                            </div>
                                        ) : null}
                                    </div>
