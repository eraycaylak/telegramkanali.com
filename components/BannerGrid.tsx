...
interface BannerGridProps {
    ...
    maxBanners?: number;
    offset?: number;
    /** Pre-shuffled banners from parent — skip fetch & shuffle when provided */
    banners?: Banner[];
}

export default async function BannerGrid({ type = 'homepage', categoryId, maxBanners, offset = 0, banners: preloadedBanners }: BannerGridProps) {
    let allBanners: Banner[];

    if (preloadedBanners) {
        // Parent already fetched & shuffled — use as-is
        allBanners = preloadedBanners;
    } else {
        // Standalone usage — fetch and shuffle ourselves
        try {
            allBanners = await getBanners(type, categoryId, true);
        } catch (e) {
            console.error('Banner fetch failed', e);
            allBanners = [];
        }

        // Fisher-Yates shuffle with Math.random() — different every page load
    ...
    if (!allBanners || allBanners.length === 0) return null;

    // Apply offset and maxBanners slicing
    let displayBanners = [...allBanners];
    if (offset > 0) {
    ...
    return (
        <section className="grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-5 my-4">
            ...
            className={`
                block relative group cursor-pointer transition-all w-full rounded-xl overflow-hidden shadow-md hover:shadow-xl hover:scale-[1.01]
                ${!banner.aspect_ratio || banner.aspect_ratio === '4:1' ? 'aspect-[4/1]' : ''}
                ${banner.aspect_ratio === '8:1' ? 'aspect-[8/1]' : ''}
                ${banner.aspect_ratio === '21:9' ? 'aspect-[21/9]' : ''}
                ${banner.aspect_ratio === '16:9' ? 'aspect-video' : ''}
                ${banner.aspect_ratio === 'auto' ? 'min-h-[150px]' : ''}
                ${!banner.image_url ? `flex items-center justify-between px-6 text-white bg-gradient-to-r ${banner.bg_color || 'from-blue-600 to-blue-400'}` : ''}
            `}
        >
            ...
            {banner.image_url && (
                <div className={`w-full relative
                    ...
                    ${banner.aspect_ratio === '21:9' ? 'aspect-[21/9]' : ''}
                    ${banner.aspect_ratio === '16:9' ? 'aspect-video' : ''}
                    ${banner.aspect_ratio === 'auto' ? 'h-full min-h-[150px]' : ''}
                `}>
                    <Image
                        ...
                        className={`object-cover ${banner.image_alignment === 'top' ? 'object-top' : banner.image_alignment === 'bottom' ? 'object-bottom' : 'object-center'}`}
                    />
                </div>
            )}
