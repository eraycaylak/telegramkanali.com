...
export default function BlogEditorClient({ post, isEditing = false, categories }: BlogEditorClientProps) {
    ...
    return (
        <div className="max-w-5xl mx-auto space-y-6">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    <div>
                        ...
                        {showPreview ? (
                            <div
                                ...
                                dangerouslySetInnerHTML={{ __html: convertTwitterLinksToEmbeds(form.content) }}
                            />
                        ) : (
                            <textarea
                                value={form.content}
                ...
                <div className="space-y-4">
                    ...
                    <div className="bg-white border border-gray-200 rounded-2xl p-4 space-y-3">
                        ...
                        <div className="flex flex-wrap gap-2 max-h-48 overflow-y-auto align-top items-start shrink-0 custom-scrollbar">
                            {categories.map((cat) => (
                                <button
                                    key={cat.id}
                                    type="button"
                                    onClick={() => setForm(f => ({ ...f, category: f.category === cat.slug ? '' : cat.slug }))}
                                    className={`px-3 py-1.5 flex items-center gap-1.5 rounded-lg text-xs font-medium transition ${form.category === cat.slug ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}
                                >
            ...
            {/* Twitter Embed Support for Admin Preview */}
            {showPreview && <TwitterEmbed url={form.content} />}
        </div>
    );
}
